...
import { Result, Prescription, LabOrder, ReferralOrder } from '@/types/types';
...
const PatientDetails: React.FC<PatientDetailsProps> = ({ patient, onBack }) => {
  ...
  const loggedInDoctor = 'Dr. John Doe'; // Replace with your logged in doctor data
        ...
        <div className="p-6">
          ...
          {activeTab === 'RX HX' && (
            <div className="mt-4">
              ...
              <RxTab prescriptions={prescriptions} onSave={handleSavePrescriptions} />
            </div>
          )}
          {activeTab === 'Action Tab' && (
            <div className="mt-4">
              <h2 className="text-2xl font-bold mb-4">Advanced Actions</h2>
              <ActionTab
                prescriptions={prescriptions}
                labOrders={labOrders}
                referralOrders={referralOrders}
                onSavePrescriptions={handleSavePrescriptions}
                onSaveLabOrders={handleSaveLabOrders}
                onSaveReferralOrders={handleSaveReferralOrders}
